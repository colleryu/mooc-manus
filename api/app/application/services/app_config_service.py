...
class AppConfigService:
    """应用配置服务"""
    def __init__(self,app_config:AppConfigRepository) -> None:
        """构造函数，完成应用配置的初始化"""
        self.app_config_repository = app_config
        ...
        try:
            ...
            for id, agent_card in agent_cards.items():
                a2a_servers.append(ListA2AServerItem(
                    ...
                    input_modes=agent_card.get("defaultInputModes") or [],
                    output_modes=agent_card.get("defaultOutputModes") or [],
                    streaming=agent_card.get("capabilities",{}).get("streaming",False),
                    push_notifications=agent_card.get("capabilities",{}).get("pushNotifications",False),
