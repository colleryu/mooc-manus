...
class BaseAgent(ABC):
    ...
    async def _invoke_llm(self, messages:List[Dict[str,Any]],format:Optional[str]=None) -> Dict[str, Any]:
            ...
            try:
                ...
                if message.get("role") == "assistant":
                    if not message.get("content") and not message.get("tool_calls"):
                        ...
                        continue
                    #6.取出非空消息并处理工具调用。DeepSeek 的 thinking 模式
                    # 要求下一轮工具结果请求原样带回上一轮 reasoning_content，
                    # 否则会返回 400。
                    ...
                    if message.get("tool_calls"):
                        #7.取出工具调用的数据，限制llm一次只能调用一个工具
                        filtered_message["tool_calls"] = message.get("tool_calls")[:1]
                else:
                    #非AI消息则记录日志，并存储message
                    logger.warning(f"LLM响应内容无法确认角色：{message.get("role")}")
        ...
        #重试次数用尽仍未获得有效响应，抛出异常避免调用方拿到None
        raise RuntimeError(f"调用LLM重试{self._agent_config.max_retries}次后仍未获得有效响应")
    ...
    async def _add_to_memory(self, messages:List[Dict[str,Any]])->None:
        ...
        #2.检查记忆的消息列表是否为空， 如果是空则需要添加预设prompt 作为初始记忆
    ...
    async def roll_back(self,message:Message ) -> None:
        """Agent的状态回滚，该函数用于确保agent的消息列表状态正确，用于发送新消息、暂停、停止任务、通知用户"""
        #1.取出记忆中的最后一条消息，检查是否工具调用
        await self._ensure_memory()
        ...
        #4.判断一下当前的工具是不是通知用户()message_ask_user
    ...
    async def invoke(self, query:str, format:Optional[str] = None)->AsyncGenerator[Event]:
        ...
        for _ in range(self._agent_config.max_iterations):
            ...
            for tool_call in message["tool_calls"]:
                ...
                #8.返回工具即将调用事件,其中tool_content比较特殊，需要在具体业务中进行调整
                yield ToolEvent(
                    tool_call_id=tool_call_id,
                    tool_name=tool.name,
                    function_name=function_name,
            ...
            #13.超过最大迭代次数，则抛出错误
            yield ErrorEvent(error=f"Agent迭代超过最大迭代次数：{self._agent_config.max_iterations},任务失败")
            return
