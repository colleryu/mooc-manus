...
1.在agent执行过程中，有可能需要多次调用Remote-Agent,
  ...
  所有需要缓存agent-card的相关信息，只有在初始化A2A客户端的时候才初始化一次，
  更新a2a服务器的时候更新，清除a2a客户端管理器时删除
2.在前端UI交互中，无论A2A服务是否启动，都会展示Card信息，
  但是呢，在执行/规划Agent中，我们只传递启用的A2A服务，所有A2A客户端必须动态接受配置；
3.一个A2A客户端会同时管理多个agent，但是不同的a2a服务有可能他们的name是一样的，
  需要考虑传递给agent信息时的唯一性，会配置多一个唯一的id；
4.由于使用httpx客户端，这个客户端需要创建上下文/释放资源，所以可以使用asyncexitStack来管理异步上下文
  ，避免大量使用 with .. as 的嵌套组合
5.A2AClientmanager的初始化非常耗时，一次请求中只初始化一次；
6.A2A配置是写在config.yaml中的并且直接暴露给开发者，有可能开发者会手动修改config.yaml， 
  所以在使用的时候，最多需要做一次校验
...
class A2AClientManager:
    ...
    async def initialize(self)->None:
        ...
        try:
            ...
            self._httpx_client = await self._exit_stack.enter_async_context(
                httpx.AsyncClient(timeout=600,trust_env=False,headers=headers)
            )
            #4.记录日志并链接所有配置的a2a服务获取卡片信息
    ...
    async def invoke(self, agent_id:str, query:str)->ToolResult:
        ...
        #3.判断端点是否存在
        if url == "":
            return ToolResult(success=False, message="该原创Agent调用端点不存在")
        try:
            #4.使用httpx客户端发起post请求传递数据
            ...
            return ToolResult(success=False,message=f"调用远程agent[{agent_id}:{url}]出错：{str(e)}")
    ...
    async def cleanup(self)->None:
        ...
        try:
            ...
            logger.error(f"清除A2A客户端管理器失败：{str(e)}")
...
class A2ATool(BaseTool):
    ...
    @tool(
        ...
        description="获取远程调用agent卡片信息，包含agent id、名称、描述、技能、请求端点等",
        parameters={},
        required=[],
    )
