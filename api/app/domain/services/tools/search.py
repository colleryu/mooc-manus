...
class SearchTool(BaseTool):
    ...
    @tool(
            ...
            parameters={
                "query":{
                    "type":"string",
                    "description":"针对搜索引擎优化的查询字符串。请提取问题中的核心实体和关键次(3-5个)，避免使用完整的自然语言问句(例如将：'北京今天的天气怎么样' 改为 '北京 天气').",
                },
                "date_range": {
                    "type": "string",
                    "enum":["all","past_hour","past_day","past_week","past_month", "past_year"],
                    "description":"(可选)搜索结果的时间范围过滤。当用户询问特定时效性的新闻或事件时(如'昨天','上周'), 必须带上此参数。默认情况为 `all`",
                },
            },
            required=["query"],
