...
class ShellTool(BaseTool):
    ...
    @tool(
        name="shell_execute",
        description="在指定 Shell 会话中执行命令。可用于运行代码、安装依赖或文件管理",
        parameters={
            "session_id":{
                "type":"string",
    ...
    @tool(
        ...
        description="查看指定Shell会话的内容，用于检查命令执行结果或监控删除",
        parameters={
            "session_id":{
                "type":"string",
                "description":"目标 Shell 会话唯一标识符"
            },
        },
        required=["session_id"]
    )
    ...
    @tool(
        ...
        parameters={
            "session_id":{
                "type":"string",
                "description":"目标 Shell 会话的唯一标识"
            },
            "input_text":{
                "type":"string",
                "description":"写入进程的输入内容"
            },
            "press_enter":{
                "type":"boolean",
    ...
    @tool(
        ...
        description="在 Shell 会话中终止正在运行的进程。用于停止长时间运行的进程或处理卡死的命令",
        parameters={
            "session_id":{
                "type":"string",
                "description":"目标 Shell 会话的唯一标识"
