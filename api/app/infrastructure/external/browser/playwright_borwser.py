...
from app.domain.external.browser import Browser as BrowserProtocal
...
def ensure_cdp_bypasses_proxy(cdp_url: str)->None:
    """Ensure Playwright's Node driver connects directly to an internal CDP host.

    Playwright inherits proxy variables from the API process. Per-session Docker
    addresses must not be sent through that proxy, otherwise CDP discovery gets
    a proxy-generated 502 response.
    """
    ...
    with _no_proxy_lock:
        ...
        for variable in ("NO_PROXY", "no_proxy"):
            entries.extend(item.strip() for item in os.environ.get(variable, "").split(",") if item.strip())
        if "*" not in entries and hostname not in entries:
            entries.append(hostname)
        value = ",".join(dict.fromkeys(entries))
        os.environ["NO_PROXY"] = value
        os.environ["no_proxy"] = value

class PlaywrightBrowser(BrowserProtocal):
    """基于此Playwright管理浏览器插件"""
    def __init__(
            self,
            cdp_url: str, #CDP的链接地址
            llm:Optional[LLM] = None, #可选参数，传递LLM，如果传递了大模型则会调用LLM对页面进行解析整理成markdown格式
        ...
        ensure_cdp_bypasses_proxy(cdp_url)
    ...
    async def _ensure_page(self)->None:
        ...
        if not self.page:
            ...
            if contexts:
                ...
                #5.判断页面是否存在
                if pages:
                    #6.获取当前最小页面(Chrome浏览器新增页面时，默往右侧添加，相当于pages中序号较大是)
                    latest_page = pages[-1]

                    #7.判断当前页面是否为最新的页面，如果不是则更新
    ...
    async def _extract_content(self)->str:
        ...
        #2.使用markdownify这个库将html文档转为markdown格式
        markdown_content = markdownify(visible_content)

        #3.模型上下文有限，提取最大不超过50k个字符
        max_content_length = min(len(markdown_content),50000)

        #4.判断分是否传递了llm，如果传递了，还可以使用llm对markdown进行整理
        if self.llm:
            #5.调用llm对markdown_content进行二次整理
            response = await self.llm.invoke(
                [
                    {
                        "role":"system",
                        "content":"你是一名专业的网页选项提取助手。请从当前页面内容中提取所有信息并将其转换为markdown格式"
                    },
                    {
                        "role":"user",
    ...
    async def _extract_interactive_elements(self)->List[str]:
        ...
        for element in interactive_elements:
            format_elements.append(f"{element['index']}:<{element['tag']}>{element['text']}</{element['tag']}>")
    ...
    async def initialize(self)->bool:
        ...
        for attempt in range(max_retries):
            try:
                ...
                #5.如果上下文存在并且第一个上下文只有一个页面则执行如下逻辑
                if contexts and len(contexts[0].pages) == 1:
                    #6.获取当前上下文的第一个页面
                    page = contexts[0].pages[0]
                    #7.判断当前页面是不是空页面，如果是则直接使用page，否则创建一个新的
                    if (
    ...
    async def cleanup(self)->None:
        ...
        try:
            ...
            #9.记录错误保存日志
            logger.error(f"清除Playwright浏览器资源出错：{str(e)}")
        finally:
            #10.重置所有资源
            self.page = None
            self.browser = None
    ...
    async def wait_for_page_load(self,timeout:int = 15)->bool:
        ...
        #2.使用异步事件循环中的时间来作为开始时间(只和异步任务相关)
    ...
    async def input(
        ...
        if coordinate_x is not None and coordinate_y is not None:
            ...
            try:
                ...
                element = await self._get_element_by_id(index)
                if not element:
                    return ToolResult(success=False,message=f"输入文本失败，该{index}元素不存在")
    ...
    async def console_view(self, max_lines:Optional[int] = None)->ToolResult:
        ...
        #2.可以执行另外一段js代码，查看控制台内容
        logs = await self.page.evaluate("""() => { return window.console.logs || [];}""")
