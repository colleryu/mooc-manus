...
from app.infrastructure.external.browser.playwright_borwser import PlaywrightBrowser
...
class DockerSandbox(Sandbox):
    ...
    def __init__(
        ...
        super().__init__()
        """构造函数，完成Docker沙箱扩展创建"""
        # Sandbox 是 API 进程直连的控制面服务。继承宿主机的 HTTP_PROXY 会让
        # localhost/容器 IP 请求偶发地被转发到代理，导致 Supervisor 健康检查
        # 超时；沙箱内浏览器是否使用代理由 SANDBOX_*_PROXY 单独控制。
    ...
    async def _resolve_hostname_to_ip(
            self,
            hostname:str,
    )->Optional[str]:
        """将docker容器主机/地址转换为ipv4格式数据"""
        try:
            #1.首先解析传递的hostname是否是ip
            try:
            ...
            #3.判断地址信息是否存在，如果存在则返回一个ipv4地址
            if addr_info and len(addr_info):
    ...
    @classmethod
    def _get_container_ip(clas,container:Model)->str:
        """根据传递的容器获取ip信息"""
        #1.获取inspect网络设置
        network_settings = container.attrs["NetworkSettings"]
        ip_address = network_settings.get("IPAddress", "")

        #3.判断容器是否存在ip，如果不存在则从networks中获取
        ...
        if not ip_address:
            raise RuntimeError(f"无法获取 Sandbox 容器[{container.name}]的网络地址")
        return ip_address
        ...
        try:
            ...
            if container is not None:
                try:
                    ...
                    logger.exception("清理创建失败的 Sandbox 容器时出错")
    ...
    async def destroy(self)->bool:
        ...
        try:
            ...
            logger.error(f"销毁当前dockersanbox[{self._container_name}]失败：{str(e)}")
            return False

    @classmethod
    async def get(cls, id:str)->Self:
        """根据传递的id创建一个连接到既有沙箱的新客户端实例。

        Sandbox 的运行环境属于会话，但 ``httpx.AsyncClient`` 属于单次 Agent
        任务。这里不能缓存 ``DockerSandbox`` 实例，否则上一轮任务关闭客户端后，
        下一轮或其他会话会拿到同一个已关闭的客户端。
        """
        #1.先获取系统配置并判断是否直连沙箱
        settings = get_settings()
        if settings.sandbox_address:
            ip = await cls._resolve_hostname_to_ip(settings.sandbox_address)
            # 直连模式的 Sandbox 由部署环境管理，不能把会话中的逻辑 ID
            # 当成 Docker 容器名，否则 cancel/delete 会误删不存在的容器。
            return DockerSandbox(ip=ip)

        #2.创建docker客户端根据容器名获取容器
        docker_client = docker.from_env()
        ...
        for attempt in range(max_retries):
            try:
                ...
                if not tool_result.success:
                    logger.warning(f"Supervisor 进程状态监控失败：{tool_result.message}")
                ...
                #9.判断是否所有服务都启动
                if all_running:
                    logger.info(f"Sandbox Supervisor所有进程服务运行正常")
                    return
                else:
                ...
                logger.error(f"无法确定Sandbox Supervisor进程状态：{str(e)}")
                await asyncio.sleep(retry_interval)
        #经过max_retries次检查后还无法确认则抛出异常
        logger.error(f"在经过{max_retries}次尝试后任无法确认Sandbox Supervisor状态信息")
        raise Exception(f"在经过{max_retries}次尝试后任无法确认Sandbox Supervisor状态信息")
    ...
    async def download_file(self, filepath)->ToolResult:
        """从沙箱中下载文件"""
    ...
    async def wait_process(self, session_id:str, seconds:int = None)->ToolResult:
        """等待沙箱中进程的执行"""
        response =  await self.client.post(
    ...
    async def kill_process(self, session_id)->ToolResult:
        """杀死沙箱中指定进程"""
        response =  await self.client.post(
