...
from app.interfaces.schemas.app_config import ListA2AServerResponse, ListMCPServerResponse
...
@router.get(
    ...
    summary="获取LLM配置信息",
    description="包含LLM提供商的base_url、temperature、model_name、max_token",
)
...
@router.post(
    ...
    response_model=Response[LLMConfig],
    summary="更新LLM配置信息",
    description="更新LLM配置信息，当api_key为空表示不更新改字段"
)
...
@router.get(
    ...
    description="包含最大迭代次数，最大尝试次数，最大搜索结果数",
)
...
async def update_agent_config(
    ...
    """更新LLM配置信息"""
    updated_llm_config = await app_config_service.update_agent_config(new_agent_config)
...
@router.get(
    ...
    description="获取当前系统的MCP服务器列表，包含MCP服务名字、工具列表、启用状态",
)
...
@router.post(
    ...
    description="根据传递的server_name + enabled 更新指定MCP服务的启用状态",
)
async def set_mcp_server_enable(
    server_name:str,
    enabled:bool = Body(...),
    app_config_service:AppConfigService = Depends(get_app_config_service),
)-> Response[Optional[Dict]]:
    """根据传递的server_name + enabled 更新服务的启用状态"""
