...
class FileReadRequest(BaseModel):
    ...
    start_line:Optional[int] = Field(
        default=None,
        description="(可选)读取的起始行，索引从0开始"
    )
    end_line:Optional[int] = Field(
        default=None,
        description="(可选)结束行号，不包含改行"
    )
...
class FileWriteRequest(BaseModel):
    ...
    leading_newline:Optional[bool] = Field(
        default=False,
        description="(可选)是否在内容开头添加前置空行"
    )
    trailing_newline:Optional[bool] = Field(
        default=False,
        description="(可选)是否在内容结尾添加后置空行"
    )
    sudo:Optional[bool] = Field(
        default=False,
        description="(可选)是否使用 sudo 权限写入"
    )
