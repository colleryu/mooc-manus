...
class ProcessInfo(BaseModel):
    ...
    stdout_logfile:str = Field(
        ...,
        description="标准输出日志文件"
    )
    stderr_logfile:str = Field(
        ...,
        description="标准错误日志文件"
    )
    pid:int = Field(
        ...,
        description="进程id(Process ID)"
    )
...
class SupervisorActionResult(BaseModel):
    ...
    start_result: Optional[Any] = Field(default=None,description="开始结果结果")
...
class SupervisorTimeoutResult(BaseModel):
    ...
    activate: bool = Field(default=False,description="超时销毁是否激活")
    ...
    remaining_seconds: Optional[float] = Field(default= None, description="超时剩余秒")