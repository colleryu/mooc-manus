...
from app.models.file import FileDeleteResult, FileFindResult, FileReadResult, FileReplaceResult, FileSearchResult, FileUploadResult,FileWriteResult,FileCheckResult
...
class FileService:
    ...
    async def read_file(
        ...
        """根据传递的文件路径 + 起始行号+ 结束行号 + 权限+最大读取文件内容"""
        try:
            #1.检测在当前权限下能否获取该文件
            ...
            #2.获取系统编码
            encoding = "utf-8"
            #3.判断是否提供sudo ，如果是sudo系统则使用命令行的显示读取文件
            if sudo :
                #使用sudo cat命令读取文件内容
                ...
                def async_read_file()->str:
                    try:
                        ...
                        raise AppException(msg=f"读文件失败：{str(e)}")

                #9.使用asyncio 创建线程读取文件
                content = await asyncio.to_thread(async_read_file)

            #10.判断是否传递了读取范围
            if start_line is not None or end_line is not None:
                #11.将内容切成行，并且提取指定范围行号的内容
    ...
    async def write_file(
        ...
        try:
            ...
            if sudo :
                #3.使用命令的方式，先向临时文件写入数据，计算追加模式
                mode = ">>" if append else ">"

                #4.创建一个临时文件
                temp_file = f"/tmp/file_write_{os.getpid()}.temp"

                #5.创建一个内部函数使用asyncio创建线程写入数据
                ...
                #11.非sudo或windows下使用python的方式写入，先确保文件路径存在
    ...
    async def replace_in_file(
        ...
        #2.计算old_str 出现的次数，只有出现次数>0才需要替换
    ...
    async def find_files(
        ...
        #2.定义一个异步函数使用asyncio子线程运行 避免io阻塞
        def async_glob():
    ...
    async def upload_file(
        ...
        try:
            ...
            logger.error(f"长传文件到沙箱失败：{str(e)}")
            raise AppException(f"长传文件到沙箱失败：{str(e)}")
