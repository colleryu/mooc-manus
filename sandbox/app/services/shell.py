...
class ShellService:
    ...
    def _format_ps1(self, exec_dir:str)->str:
        """格式化命令格式展示，增强交互体验。例如：root@myserver:/var/log $"""
    ...
    async def _create_process(cls, exec_dir:str, command:str)->asyncio.subprocess.Process:
        ...
        #1.ubuntu统一使用/bin/bash这个解释器
        logger.debug(f"在目录{exec_dir}下使用命令{command}创建一个子进程")
        shell_exec = "/bin/bash"

        #3.创建一个系统及的子进程用来执行shell命令
    ...
    def get_console_records(self, session_id:str)->List[ConsoleRecord]:
        """从指定会话中获取控制台记录"""
        #1.判断传递的会话是否存在
        logger.debug(f"正在获取Shell会话中的控制台记录：{session_id} ")
    ...
    async def _start_output_reader(self,session_id:str, process: asyncio.subprocess.Process)->None:
        ...
        while True:
            ...
            if process.stdout:
                try:
                    #4.读取缓存的数据，假设一次读取4096
                    buffer = await process.stdout.read(4096)
                    if not buffer:
                        break
                    #5.使用编码器进行编码同时设置final=False
                    ...
                    logger.error(f"读取进程输出时出错：{str(e)}")
                    break
            else:
                break
    ...
    async def wait_process(self, session_id:str, seconds:Optional[int]=None)->ShellWaitResult:
        ...
        try:
            ...
            #4.记录日志并返回等待结果
            logger.info(f"进程已经完成，返回代码为：{process.returncode}")
    ...
    def create_session_id(cls)->str:
        ...
        logger.info(f"创建一个新的Shell会话：{session_id}")
        return session_id
    ...
    async def exec_command(
        ...
        try:
            ...
            if session_id not in self.active_shells:
                ...
                #5.创建后台任务来运行输出读取器
                reader_task = asyncio.create_task(self._start_output_reader(session_id,process))
            else:
                #6.该会话已经存在则读取数据
                logger.debug(f"使用现有的Shell会话：{session_id}")
                shell = self.active_shells[session_id]
                old_process = shell.process
                #7.判断旧进程是否还在运行，如果是则停止再运行新命令
                if old_process.returncode is None:
                    logger.debug(f"正在结束会话中的上一个进程：{session_id}")
                    try:
                        #8.结束旧进程并优雅等待1s
                        ...
                        logger.warning(f"强制终止Shell会话中的进程 {session_id}")
                        old_process.kill()
                #10.关闭之后传教一个新的进程
                process = await self._create_process(exec_dir,command)
            ...
            try:
                ...
                if wait_result.returncode is not None:
                    ...
                    # 等待后台输出读取器把进程的完整输出消费完，避免读到未刷新的内容
                    await reader_task
                ...
                #16.等待超时，记录日志不做额外处理让命令再后台运行
                logger.warning(f"进程再会话超时后任再运行：{session_id}")
                pass
            except Exception as e:
                #17.其他异常忽略让程序继续运行
    ...
    async def write_shell_input(
        ...
        try:
            ...
            if process.returncode is not None:
                logger.error(f"子进程已经结束，无法写入：{session_id}")
                raise BadRequestException(f"子进程已经结束，无法写入：{session_id}")

            #4.ubuntu系统统一使用utf-8 与 \n
            encoding = "utf-8"
            line_ending = "\n"
            ...
            #6.将字符串编码成字节流(发送给进程使用)
            input_date = text_to_send.encode(encoding)
            ...
            #8.向子进程写入数据
            process.stdin.write(input_date)
        ...
        try:
            ...
            if process.returncode is None:
                ...
                try:
                    ...
                    process.kill()
                #7.记录日志并且返回结果
                logger.info(f"进程成功终止，返回代码为：{process.returncode}")
                return ShellKillResult(status="terminated",returncode=process.returncode)
            else:
                #8.进程已经关闭无需重复关闭
                logger.info(f"进程已经结束，返回代码：{process.returncode}")
