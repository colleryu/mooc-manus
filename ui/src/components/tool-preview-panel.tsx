...
function ToolKindIcon({ kind }: { kind: ToolKind }) {
  ...
  if (kind === 'bash') return <Terminal {...props} />
  if (kind === 'browser') return <Globe {...props} />
  if (kind === 'search') return <Search {...props} />
  if (kind === 'file') return <FileSearch {...props} />
  if (kind === 'mcp') return <Wrench {...props} />
  if (kind === 'a2a') return <Bot {...props} />
      ...
      <div className="flex-1 rounded-lg overflow-hidden border min-h-0 relative">
        {screenshot ? (
          <ScrollArea className="h-full">
            {/* eslint-disable-next-line @next/next/no-img-element -- 后端截图流使用动态下载 URL */}
            <img
              src={screenshot}
              alt="浏览器截图"
